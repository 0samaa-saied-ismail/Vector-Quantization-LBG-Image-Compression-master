...
public class ImageRW {
    ...
    public static int[][] readImage(String path) {
        BufferedImage img;
        try {
            img = ImageIO.read(new File(path)); // read img from path
            height = img.getHeight(); // get high and width
            width = img.getWidth();
            int[][] imagePixels= new int[height][width]; // set img in 2d array
            for (int x = 0; x < width; x++) {
                for (int y = 0; y < height; y++) {
                    int pixel = img.getRGB(x, y); // get pixel color by position
                    int red = (pixel & 0x00ff0000) >> 16; // take pixel and shift right 16 places to get red
